...
The dataset is frequently updated. The analysis below is based on datasets last updated on **July 22, 2020**: 
...
* We drop the very last reported week of 2020 for CHE, FIN, LTU, NOR, ISR, SVK, and USA which have steep declines due to incomplete reporting.
...
deaths = deaths[-which(CountryCode=="ISR" & Year==2020 & Week == 22)]
deaths = deaths[-which(CountryCode == "USA" & Year==2020 & Week %in% 26:27)]
deaths = deaths[-which(CountryCode == "NOR" & Year==2020 & Week %in% 26:28)]
deaths = deaths[-which(CountryCode == "FIN" & Year==2020 & Week==27)]
deaths = deaths[-which(CountryCode == "CHE" & Year==2020 & Week %in% 26:27)]
```
...
for (i in 1:length(countryUnq)){
  ...
  fig <- fig %>% add_lines(x = weeksForecast, y = xPred$yhat,
   ...
   fig <- fig %>% add_lines(x = 1:26, y = med5Yr$med[1:26],
                           mode = "lines",
                           showlegend = FALSE,
                           hovertemplate = paste(
...
levels(excess$CountryCode) = c("Austria","Belgium","Bulgaria","Switzerland","Czech Republic","Germany","Denmark","Spain","Estonia", "Finland","France","Scotland","England & Wales","Hungary","Israel","Italy","Lithuania","Netherlands","Norway",
