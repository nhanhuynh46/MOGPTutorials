...
<div class="col2">
  ...
  <img src="https://www.countryflags.io/IL/shiny/24.png"> Israel (ISL) \
...
The dataset is frequently updated. The analysis below is based on datasets last updated on **August 05, 2020**: 
...
* We drop the very last reported week of 2020 for CHE, FIN, LTU, NOR, ISR, SVK, SWE and USA which have steep declines due to incomplete reporting.
...
deaths = deaths[-which(CountryCode=="SVK" & Year==2020 & Week %in% 25:26)]
deaths = deaths[-which(CountryCode=="LTU" & Year==2020 & Week == 24)]
deaths = deaths[-which(CountryCode=="ISR" & Year==2020 & Week == 28)]
deaths = deaths[-which(CountryCode == "USA" & Year==2020 & Week == 29)]
deaths = deaths[-which(CountryCode=="SWE" & Year==2020 & Week == 30)]
deaths = deaths[-which(CountryCode == "NOR" & Year==2020 & Week %in% 29:30)]
deaths = deaths[-which(CountryCode == "FIN" & Year==2020 & Week==29)]
deaths = deaths[-which(CountryCode == "CHE" & Year==2020 & Week %in% 27:28)]
```
...
(2) perform out-of-sample predictions in 2020 starting from Week 5 to Week 30 (end of July 2020). 
...
for (i in 1:length(countryUnq)){
  ...
  fig <- fig %>% add_lines(x = weeksForecast, y = xPred$yhat,
   ...
   fig <- fig %>% add_lines(x = 1:30, y = med5Yr$med[1:30],
                           mode = "lines",
                           showlegend = FALSE,
                           hovertemplate = paste(
...
levels(excess$CountryCode) =c("Austria",
                              "Belgium",
                              "Bulgaria",
                              "Switzerland",
                              "Czech Republic",
