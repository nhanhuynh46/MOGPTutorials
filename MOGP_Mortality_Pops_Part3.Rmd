...
In [Part I](MOGP_Mortality_Pops_Part1.html) and [Par II](MOGP_Mortality_Pops_Part2.html), we mainly focus on the use of MOGP model for _isotropic data_, or when all $L$ populations have the same set of inputs. In this tutorials, we will illustrate how to generalize this framework when we have missing data or when populations have different set of inputs. 
...
<p>&nbsp; 
  ...
  * $\color{blue}{\texttt{covICM_Notched.Rda}}$: to define ICM with partially heterotropic data where only a portion of $L$ inputs are available. This is useful as the reported data from different countries in the HMD arrives non-synchronously. For instance, as of March 2020 some countries already have 2018 data added, most have data up to 2017, and a few are still lagging and only have data up to 2016. The prevailing approach is to consider the time period that is common to all countries that are being modeled. This implies that the most recent observations may be dropped for some countries. Of course, such recent data is in fact the most informative for picking up new insights about the present longevity trend. Note that the HMD datasets are updated continuously, so that which datsets have the latest observations changes dynamically over time.\
<p>&nbsp; 
```{r, cache = TRUE, echo = FALSE, fig.align = "center"}
library(plotly)
library(ggplot2)

country = c("Austria", "Belarus", "Czechia", "Denmark", "Estonia", "France", "Germany", "Hungary", "Latvia", "Lithuania", "Netherlands", "Poland", "Spain", "Sweden", "Switzerland", "UK")
minYear = c(1950, 1959, 1950, 1950, 1959, 1950, 1990, 1950,
            1959, 1959, 1950, 1958, 1950, 1950, 1950, 1950)
maxYear = c(2017, 2016, 2017, 2016, 2017, 2017, 2017, 2017,
            2017, 2017, 2016, 2016, 2016, 2018, 2016, 2016)
col = c("antiquewhite2","aquamarine4","blue1","plum1",
        "red","burlywood3","cadetblue1","#C3D7A4",
        "cornflowerblue","#FFDB6D","darkorchid2","deeppink",
        "dimgrey","green","darkorange","black")


fig <- plot_ly(hovertemplate = paste('%{y}', '<br>Year: %{x}<br>'),
               name = "")
for (i in 1:16){
  fig <- fig %>% add_markers(x = minYear[i]:maxYear[i], 
                             y = country[i], 
...
fig <- fig %>% layout(xaxis = list(titlefont = list(size=20), 
                      showgrid = F, 
                      showline = TRUE,
                      ticks = "outside",
                      range = c(1948,2021)),
                      yaxis = list(showgrid = F, 
                                   showline = FALSE,
                                   range = c(-0.5,17)),
                      title = list(text = "Data availability",
                                   x = 0.1,
                                   y = 0.99),
                      autosize = F)
fig <- fig %>% add_annotations(x = 2005, y = 16.25,
                               text = "Date of Pull from the HMD: March, 2020", 
                               showarrow = FALSE,
...
kable_styling(df_html,bootstrap_options = "striped", full_width = F)
```
`r icon::fa_thumbs_up(colour = "#059b33", size = 1)` &nbsp; $\text{SMAPE}_{SOGP}=1.575 > \text{SMAPE}_{MOGP}=0.947$. This illustrates how we boosts the predictive power for Hungarian Males by incorporating most recent data from Estonia. 
...
fig <- fig %>% layout(xaxis = list(title = 'Age', 
                                   ...
                                   showline = TRUE, 
                                   range = c(69,85)),
                      yaxis = list(title = 'Standard Deviation of f*', 
                                   titlefont = list(size=20), 
                                   ticks = "outside", 
                                   showgrid = F, 
                                   showline = TRUE),
...
<p>&nbsp; 
<p>
`r icon::fa_thumbs_up(colour = "#059b33", size = 1)` &nbsp; Borrowing information from Estonia also increases the forecast credibility: the standard deviation of the latent function $f$ at new inputs is smaller in MOGP than SOGP model. Note that adding more data that are highly correlated with Hungarian mortality data will further improve the accuracy and the credibility of the prediction. 
