...
* We drop the very last reported week of 2020 for FIN, NOR and USA which have steep declines due to incomplete reporting.

* We drop Iceland which has different seasonality and much more noisy data due to very small population (more than 10 times smaller than all other countries considered)
...
```{r, echo = FALSE, include = FALSE, SpainModify}
# modify Spain data:
deaths = deaths[CountryCode=="ESP" & Year==2019 & Week %in% 40:52, y:= y - 1.859]
deaths = deaths[CountryCode=="ESP" & Year==2020, y:=y-1.859]
# mort:
deaths = deaths[CountryCode=="ESP" & Year==2019 & Week %in% 40:52, RTotal:= exp(y)]
deaths = deaths[CountryCode=="ESP" & Year==2020, RTotal:=exp(y)]
```


```{r, removeLastWeek, echo = FALSE}
deaths = deaths[!CountryCode %in% c("ISL")]
# remove the last week data point in 2020 for several country:
deaths = deaths[-which(CountryCode=="USA" & Year==2020 & Week %in% c(18,19))]
deaths = deaths[-which(CountryCode=="NOR" & Year==2020 & Week %in% c(18,19))]
deaths = deaths[-which(CountryCode=="FIN" & Year==2020 & Week==19)]
```
...
levels(excess$CountryCode) = c("Austria","Belarus","Bulgaria","Czech Republic","Germany","Denmark",
                               "Spain","Finland","England & Wales","Netherlands","Norway",
                               "Portugal","Sweden","USA")
```

```{r, echo=FALSE}
excessSum <- data.frame(Country=unique(excess$CountryCode),excessTotal=rep(0,14),lastWeek=rep(0,14))
