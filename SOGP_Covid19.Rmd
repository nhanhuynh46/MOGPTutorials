...
The dataset is frequently updated. The analysis below is based on datasets last updated on **June 26, 2020**: 
...
* We drop the very last reported week of 2020 for BEL, FIN, HUN, NOR, ESP, SVK, SWE and USA which have steep declines due to incomplete reporting.
...
# remove the last week data point in 2020 for several countries Week %in% c(21,22,23):
deaths = deaths[-which(CountryCode=="USA" & Year==2020 & Week == 23)]
deaths = deaths[-which(CountryCode=="SVK" & Year==2020 & Week == 18)]
deaths = deaths[-which(CountryCode=="SWE" & Year==2020 & Week %in% c(23,24))]
deaths = deaths[-which(CountryCode=="HUN" & Year==2020 & Week ==21)]
deaths = deaths[-which(CountryCode %in% c("ESP", "FIN","NOR","BEL") & Year==2020 & Week ==23)]
```
...
levels(excess$CountryCode) = c("Austria","Belgium","Bulgaria","Czech Republic","Germany","Denmark",
