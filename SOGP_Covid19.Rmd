...
The dataset is frequently updated. The analysis below is based on datasets last updated on **June 07, 2020**: 
...
* We drop the very last reported week of 2020 for FIN, FRATNP, NOR and USA which have steep declines due to incomplete reporting.

* We drop Iceland and Slovakia which has different seasonality and much more noisy data due to very small population (more than 10 times smaller than all other countries considered)
...
deaths = deaths[!CountryCode %in% c("ISL","SVK")]
# remove the last week data point in 2020 for several country:
deaths = deaths[-which(CountryCode=="USA" & Year==2020 & Week %in% c(19,20))]
deaths = deaths[-which(CountryCode=="NOR" & Year==2020 & Week %in% c(20,21))]
deaths = deaths[-which(CountryCode=="FIN" & Year==2020 & Week==20)]
deaths = deaths[-which(CountryCode=="FRATNP" & Year==2020 & Week %in% c(17,18))]
```
...
levels(excess$CountryCode) = c("Austria","Belarus","Bulgaria","Czech Republic","Germany","Denmark",
                               "Spain","Finland","France","England & Wales","Hungary","Italy","Netherlands","Norway",
                               "Portugal","Sweden","USA")
```

```{r, echo=FALSE}
excessSum <- data.frame(Country=unique(excess$CountryCode),excessTotal=rep(0,17),lastWeek=rep(0,17))
