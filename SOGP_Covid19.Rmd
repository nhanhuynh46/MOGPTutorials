...
<div class="col2">
  ...
  <img src="https://www.countryflags.io/IE/shiny/24.png"> Northern Ireland (GBR_NIR)\
  <img src="https://www.countryflags.io/GB/shiny/24.png"> Scotland (GBR_SCO) \
  <img src="https://www.countryflags.io/GR/shiny/24.png"> Greece (GRC) \
  <img src="https://www.countryflags.io/HR/shiny/24.png"> Croatia (HRV) \
  ...
  <img src="https://www.countryflags.io/IL/shiny/24.png"> Israel (ISR) \
  ...
  <img src="https://www.countryflags.io/LV/shiny/24.png"> Latvia (LVA) \
  ...
  <img src="https://www.countryflags.io/PL/shiny/24.png"> Poland (POL) \
  <img src="https://www.countryflags.io/PT/shiny/24.png"> Portugal (PRT)\
  <img src="https://www.countryflags.io/RU/shiny/24.png"> Russia (RUS)\
  <img src="https://www.countryflags.io/SK/shiny/24.png"> Slovakia (SVK)\
  <img src="https://www.countryflags.io/SI/shiny/24.png"> Slovenia (SVN)\
...
The dataset is frequently updated. The analysis below is based on datasets last updated on **September 09, 2020**: 
...
deaths = deaths[!CountryCode %in% c("ISL","LUX","RUS")]
# remove the last week data point in 2020 for several countries Week %in% 34:36
deaths = deaths[-which(CountryCode == "CHE" & Year==2020 & Week %in% 34:35)]
deaths = deaths[-which(CountryCode == "FIN" & Year==2020 & Week== 35)]
deaths = deaths[-which(CountryCode == "NOR" & Year==2020 & Week %in% 35:36)]
deaths = deaths[-which(CountryCode=="SWE" & Year==2020 & Week == 36)]
deaths = deaths[-which(CountryCode == "USA" & Year==2020 & Week == 34)]
```
...
lengthscales = c()
for (i in which(!countryUnq %in% c("GBRTENW","GBR_NIR"))){
  lengthscales = rbind(lengthscales,mortSingle_nug[[i]]@covariance@range.val)
}

# fix lengthscale in Year and Week for England and Wales
...
# fix lengthscale in Year for Northern Ireland
i = which(countryUnq=="GBR_NIR")
mortSingle[[i]] = km(formula = ~ x.year + cos(2*pi/52*(x.week-4)) + sin(2*pi/52*(x.week-4)) + cos(4*pi/52*(x.week-4)) + sin(4*pi/52*(x.week-4)), 
                       design = mortSingle_nug[[i]]@X, 
                       response = mortSingle_nug[[i]]@y,
                       noise.var = rep(nug[i],mortSingle_nug[[i]]@n), 
                       coef.trend = mortSingle_nug[[i]]@trend.coef,  
                       coef.cov = c(mean(lengthscales[,1]),mortSingle_nug[[i]]@covariance@range.val[2]),
                       coef.var = mortSingle_nug[[i]]@covariance@sd2, 
                       covtype = mortSingle_nug[[i]]@covariance@name)
```
...
xPred = xPred[-which(year==2020 & week %in% 38:52)]
  ...
  if (length(ex)==0){
    ...
    fig <- fig %>% add_markers(x = length(obs20$y), y = obs20$y[length(obs20$y)], 
                           ...
                           hovertemplate = paste(
                            ...
                            "Log-mort: %{y:.4f}<br>",
                            "<extra></extra>"))
    fig <- fig %>% add_annotations(x = length(obs20$y) + 3, y = obs20$y[length(obs20$y)]+0.005,
                               showarrow = FALSE,
                               text = paste("Wk ",length(obs20$y)),
                               font = list(color = '#264E86',
                                           family = 'sans serif',
...
browsable(div(style = "display: flex;  justify-content: center; height: calc(50vh - 50px); gap: 40px",
        ...
        div(ggplotly(P[[23]]) %>% config(displayModeBar = F)),
        div(ggplotly(P[[24]]) %>% config(displayModeBar = F))))
```
```{r, echo = FALSE}
browsable(div(style = "display: flex;  justify-content: center; height: calc(50vh - 50px); gap: 40px",
        div(ggplotly(P[[25]]) %>% config(displayModeBar = F)),
        div(ggplotly(P[[26]]) %>% config(displayModeBar = F)),
        div(ggplotly(P[[27]]) %>% config(displayModeBar = F))))
```

```{r, echo = FALSE}
browsable(div(style = "display: flex;  justify-content: center; height: calc(50vh - 50px); gap: 40px",
        div(ggplotly(P[[28]]) %>% config(displayModeBar = F)),
        div(ggplotly(P[[29]]) %>% config(displayModeBar = F))))
```
...
for (i in 1:length(countryUnq)){
  ...
  fig <- fig %>% add_lines(x = weeksForecast, y = xPred$yhat,
   ...
   fig <- fig %>% add_lines(x = 1:37, y = med5Yr$med[1:37],
                           mode = "lines",
                           showlegend = FALSE,
                           hovertemplate = paste(
  ...
  #                                            family = 'sans serif',
  #                                            size = 12))
  fig <- fig %>% add_annotations(x = 12, y = min(xPred$l95),
                                 showarrow = TRUE,
                                 text = "",
                                 ax = 5.25,
                                 ay = min(xPred$l95),
                                 ...
                                 arrowcolor = '#264E86',
                                 arrowsize = .5)
  fig <- fig %>% add_annotations(x = 8.5, y = min(xPred$l95) + 0.025,
                                 showarrow = FALSE,
                                 text = "Projected",
                                 font = list(color = '#264E86',
                                             family = 'sans serif',
